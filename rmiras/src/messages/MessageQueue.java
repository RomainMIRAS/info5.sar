...
public class MessageQueue implements IMessageQueue {
	...
	private class MyReadListener implements IChannel.ReadListener {
		...
		public void available() {
			...
			switch (state) {
				case READING_LENGTH -> {
								byteRead = channel.read(length, offset, 4 - offset);
								offset += byteRead;
								if (offset == 4) {
									int len = 0;
									for (int i = 0; i < 4; i++) {
				...
				case READING_MSG -> {
								byteRead = channel.read(data, offset, data.length - offset);
								offset += byteRead;
		...
		public void written(int bytesWritten) {
			...
			case WRITING_MSG -> {
                            if (offset == currentSendingMessage.getLength()) {
                                ...
                                channel.write(currentSendingMessage.getBytes(), offset, currentSendingMessage.getLength() - offset,
                                        this);
                            }
                        }
			}
